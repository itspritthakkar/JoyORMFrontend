...
import { useTaskHelperContext } from 'views/user/task/contexts/TaskHelperContext';
import useTaskService from 'views/user/task/services/useTaskService';
...
const EditTaskHeader = () => {
    ...
    useEffect(() => {
        const match = options.find((opt) => opt.id === currentTask.data.assignedByUserId) || options[0];
                ...
                {users.isLoaded && !users.isError && (
                    <Autocomplete
                        ...
                        onChange={(event, newValue) => {
                            ...
                            updateExistingTask({ ...currentTask.data, assignedByUserId: newValue.id });
                        }}
                        renderInput={(params) => (
                            <TextField
                                {...params}
